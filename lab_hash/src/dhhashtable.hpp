...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    unsigned int original_hash_index = hashes::hash(key, size);
    unsigned int secondary_hash_val = hashes::secondary_hash(key, size);
    unsigned int hash_index = original_hash_index;
    int k = 1; //which bucket are we on?
    while (table[hash_index] != NULL) {
        hash_index = (original_hash_index + k * secondary_hash_val) % size;
        k++;
    }
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while (m > 0) {
        ...
        } else if (should_probe[hash_index] == 0) {
            return -1;
        }
        hash_index = (hash_index + k * secondary_hash_val) % size;
        k++;
        m--;
    }
    return -1;
