...
HuffmanTree::HuffmanTree(vector<Frequency> frequencies)
{
    ...
    std::cout << "Hi24" << std::endl;
    buildTree(frequencies);
    std::cout << "Hi25" << std::endl;
    vector<bool> path;
    buildMap(root_, path); //line is failing
    std::cout << "Hi26" << std::endl;
}
                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
    TreeNode* s_front = NULL;
    TreeNode* m_front = NULL;

    if (!singleQueue.empty()) {
        s_front = singleQueue.front();
    }
    if (!mergeQueue.empty()) {
        m_front = mergeQueue.front();
    }
    if (s_front == NULL) {
        mergeQueue.pop();
        return m_front;
    } else if (m_front == NULL) {
        singleQueue.pop();
        return s_front;
    } else {
        if(s_front -> freq.getFrequency() <= m_front->freq.getFrequency()) {
            singleQueue.pop();
            return s_front;
        } else {
            mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while ((singleQueue.size() + mergeQueue.size()) > 1) {
        TreeNode* removedNode1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* removedNode2 = removeSmallest(singleQueue, mergeQueue);

        TreeNode* node = new TreeNode(removedNode1->freq.getFrequency() + removedNode2->freq.getFrequency());
        node->left = removedNode1;
        node->right = removedNode2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    TreeNode* treeNode;
    if (b) {
        treeNode = new TreeNode(Frequency(bfile.getNextByte(), 0));
    } else {
